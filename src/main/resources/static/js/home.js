...
async function saveUserInMongo(payload) {
  try {
    const response = await fetch(
      "http://3.0.102.63:7074/exuser/validateUserCreation",
      {
        method: "POST",
        headers: {
...
async function getAllChild(id, usertype, currentPage, itemsPerPage) {
  const response = await fetch(
    `http://3.0.102.63:7074/exuser/${id}/${usertype}?pageNumber=${currentPage}&pageSize=${itemsPerPage}`
  );
...
async function showSettingPopup(userid, statusOfUser) {
  ...
  submitBtn.addEventListener("click", async function () {
    ...
    if (password !== "") {
      ...
      try {
        const response = await fetch(
          `http://3.0.102.63:7074/exuser/action/${action}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
async function showPopup(currentBalance, userid, onPopupDisplay) {
  ...
  submitBtn.addEventListener("click", async function () {
    ...
    if (newValue !== "" && password !== "") {
      ...
      try {
        const response = await fetch(
          "http://3.0.102.63:7074/exuser/creditReference",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
async function showImpMessage() {
  const response = await fetch(
    "http://3.0.102.63:7074/exuser/currentImportantMessage"
  );
  const result = await response.json();
  const data = JSON.parse(decryptMessage(result.data));
...
async function searchUser(currentPage, itemsPerPage, userId) {
  ...
  const response = await fetch(
    `http://3.0.102.63:7074/exuser/search/${id}/${usertype}?pageNumber=${currentPage}&pageSize=${itemsPerPage}&userid=${userId}`
  );
  const result = await response.json();
  const encryptedData = result.data;
  var decryptData = JSON.parse(decryptMessage(encryptedData));
